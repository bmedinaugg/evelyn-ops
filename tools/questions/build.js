...
const CHAINS = {
  ...
  faq_store: {
    ...
    steps: [
      ...
      'Feed 2 — a second process writes 58 more articles every morning and nobody has traced what it is. Recorded as TrainMore FAQs.xlsx until 14 Sep 2026; that was an inference from column shape and the file was never found.',
    ...
    ends_at: 'Member Care, for two of the three feeds. Publishing a Freshdesk article or adding a row to bot.manual_faqs reaches the bot the next morning with no deploy. The second feed has no known owner at all.',
    caveat: 'The second feed repeats articles that already sync from Freshdesk, so most answers are stored twice and retrieval can return the same one twice. Editing the Freshdesk article does not change the copy, and there is no known way to edit the copy.',
  },
  magicline: {
...
const QUESTIONS = [
  ...
  },
  {
    ...
    caveat: 'This is the clearest case of the duplicate-storage problem: the booking articles exist twice in the store, once from the Freshdesk sync and once from the untraced second feed.',
  },
  {
